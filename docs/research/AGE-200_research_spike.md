...
- X-marker 是独立问题。AGE-131 旧案例在第二份录屏上把无锚点扫描从约 44/62 误报降到 8/62，仍残留静态噪声（`../case-studies/AGE-131_AGE-136_case_study.md:11`）。当前代码已经落地更强的 counter-window 锚定、多帧基线、位置持续性和黑名单上限，并在 docstring 诚实记录约 7% crop 黑名单覆盖可能造成漏检（`video_utils.py:724-786`）。但生产 orchestrator 目前没有传 `respawn_reader`，所以已标定的复活倒计时共现特征并未在该调用链启用；函数仍退化为特征 1+2。
