...
def load(p:Path,default:Any): return json.loads(p.read_text(encoding='utf-8')) if p.exists() else default
...
def main():
    ...
    eligible=[r for r in seeds.get('records',[]) if r.get('seed_eligibility')=='eligible-seed']; boot=[]; windows=[]; raw_by_seed={}
    ...
    hints=[]; manifests=[]
    for src in eligible:
        sid,vid=src['seed_id'],src['video_id']; media=root/'data/evaluation/replay_seeds/media'/f'{vid}.webm'; probe=probe_media(media); ready=probe['status']=='complete'; stage={'acquisition':'complete' if media.exists() else 'blocked_missing_media','media_probe':probe['status'],'bootstrap_labels':'available_silver_remote_multimodal' if boot and any(b['seed_id']==sid for b in boot) else 'unavailable','speech_to_text':'pending','caption_ocr':'pending','ocr_stt_alignment':'pending','gameplay_reference_alignment':'pending','detectors':'pending_execution' if ready else 'blocked_missing_media'}
        manifest={'schema_version':'seed-source-v4','generator_version':'claim-model-v4-bootstrap-canonical','seed_id':sid,'video_id':vid,'url':src.get('url'),'title':src.get('title'),'hero':src.get('hero'),'role':src.get('role'),'series':src.get('series'),'rank_profile':src.get('rank_profile'),'artifact_path':str(media.relative_to(root)) if media.exists() else None,'artifact_sha256':sha256(media),'media_probe':probe,'timestamp_contract':{'captured_media_presentation_time':'unknown','normalized_derivative_time':'unknown','reviewer_speech_caption_time':'unknown','referenced_gameplay_time':'unknown'},'stage_status':stage,'terminal_status':'media_ready_transcript_pending' if ready else ('blocked_missing_media' if not media.exists() else 'media_probe_failed')}; manifests.append(manifest); dump(out/'videos'/sid/'source_manifest.json',manifest); dump_jsonl(out/'videos'/sid/'transcript'/'speech_segments.jsonl',[]); dump_jsonl(out/'videos'/sid/'transcript'/'ocr_observations.jsonl',[]); dump_jsonl(out/'videos'/sid/'claims.jsonl',[]); dump_jsonl(out/'videos'/sid/'fixture_candidates.jsonl',[])
        if target and sid==target['seed_id']:
            for i,r in enumerate(reconstructed):
                hid=f'{sid}_zh_hint_{i:04d}'; context=context_only(r['text'],r.get('gameplay_state') or ''); hint={'hint_id':hid,'seed_id':sid,'video_id':vid,'start_sec':r['start_sec'],'end_sec':r['end_sec'],'raw_text':r['text'],'language':'zh','source_video_language':'zh','representation':'windowed_multimodal_caption_extraction','canonical':False,'label_tier':'silver_windowed_multimodal','confidence':r['confidence'],'gameplay_state':r.get('gameplay_state'),'context_only':context,'verification_status':'unverified','promotion_rule':'requires_deterministic_ocr_or_stt_verification'}; hints.append(hint); dump_jsonl(out/'videos'/sid/'transcript'/'windowed_multimodal_caption_hints.jsonl',[hint])
    # Rewrite the per-video hint file once so repeated records are not overwritten.
    if target:
        dump_jsonl(out/'videos'/target['seed_id']/'transcript'/'windowed_multimodal_caption_hints.jsonl',[h for h in hints if h['seed_id']==target['seed_id']])
    ...
    dump_jsonl(out/'aggregates'/'canonical_commentary_segments.jsonl',[]); dump_jsonl(out/'aggregates'/'canonical_claims.jsonl',[]); dump_jsonl(out/'aggregates'/'evidence_timeline.jsonl',[]); dump_jsonl(out/'aggregates'/'detector_fixtures.jsonl',[]); dump_jsonl(out/'aggregates'/'manifests.jsonl',manifests)
    missing=[{'capability':c,'implementation_status':s,'execution_status':'not_run_missing_detector','fixture_count':0} for c,s in CAPABILITIES.items() if s=='capability_missing']; dump_jsonl(out/'aggregates'/'missing_capability_queue.jsonl',missing)
    summary={'schema_version':'handoff-corpus-v4','generator_version':'claim-model-v4-bootstrap-canonical','eligible_seed_count':len(eligible),'bootstrap':{'english_ai_paraphrases':len(boot),'candidate_windows':len(windows),'chinese_windowed_caption_hints':len(hints),'chinese_context_only_hints':sum(h['context_only'] for h in hints)},'canonical':{'chinese_ocr_observations':0,'chinese_commentary_segments':0,'claims':0,'reviewer_observations':0,'event_fixtures':0},'canonical_language':'zh','bootstrap_language':'en','canonical_promotion_policy':'Only deterministic OCR/STT or fused verified intervals may enter canonical aggregates','gameplay_time_policy':'unknown_until_gameplay_reference_alignment','capability_fixture_counts':{}}
